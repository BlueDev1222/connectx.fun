...
Server handlers validate users with `getUser`; the Next.js proxy refreshes SSR cookies using verified claims. Account and role status are read from the DB, not from editable cookies. DB mutation limits are transactional: 90 commands/minute, 8 posts/minute, 20 messages/minute, 5 reports/minute, 3 support tickets/minute, 2 new communities/servers per minute, and 6 uploads/minute per account. Limits are not IP/device identity verification and do not replace provider-level bot protection.
...
- Media POST checks the exact configured Origin, Auth user, rate limit, size, declared MIME, extension, and binary signature. Filenames are generated UUIDs under the owner's UUID.
- Storage is private and has no client upload/update policy. The server-only key uploads only after validation. Reads require current RLS authorization and use five-minute signed URLs. A previously issued signed URL remains usable until expiry after a privacy change.
...
- Security headers include CSP, frame denial, content-type sniffing prevention, referrer policy, and restricted device permissions. Production CSP omits unsafe-eval but permits inline Next.js scripts; nonce-based CSP can be added by the deployment operator.

## Deletion and retention

Account deletion requires a same-origin POST and explicit confirmation. The handler refuses deletion while the user owns a community or is the platform owner. It deactivates the profile, deletes uploaded files, removes owned server profiles, revokes sessions, then deletes the Auth user. Cascades remove profile data, authored posts, reactions, settings, and conversations/messages. Audit actor references are set null. If a service fails, the account remains inactive for safe staff recovery; deletion across Auth/Storage/Postgres is not an atomic distributed transaction.
